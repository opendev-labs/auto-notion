    ...
    return (
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {stats.map((stat, i) => (
                    <div key={i} className="glass p-6 rounded-lg flex items-center gap-4 group">
                        <div className="p-3 rounded-md bg-white/5 border border-white/10 group-hover:bg-white group-hover:text-black transition-all">
                            <stat.icon size={20} />
                        </div>
                        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {modules.map((module) => (
                    <motion.div
                        key={module.id}
                        whileHover={{ y: -5 }}
                        className="glass-dark rounded-lg p-8 border border-white/10 relative overflow-hidden group"
                    >
                        <div className="flex justify-between items-start mb-8">
                            <div className="flex items-center gap-4">
                                <div className="p-4 rounded-md bg-white/5 text-white/60 group-hover:bg-white group-hover:text-black transition-all duration-500">
                                    <module.icon size={24} />
                                </div>
                                <div>
                            ...
                            <NavLink
                                to={module.path}
                                className="p-3 rounded-lg bg-white/5 text-white/20 hover:text-white hover:bg-white/10 transition-all"
                            >
                        ...
                        <NavLink to={module.path} className="mt-8 w-full block text-center py-4 rounded-lg bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white hover:bg-white/10 transition-all">
                            Enter Full Module
                        </NavLink>
                    </motion.div>
                ))}
            ...
            <div className="glass-dark rounded-lg p-10 border border-white/10">
                <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                    ...
                    <div className="flex gap-4">
                        <NavLink to="/settings" className="px-8 py-4 bg-white text-black rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-zen-sage transition-all">
                            System Settings
                        </NavLink>
                        <NavLink to="/logs" className="px-8 py-4 bg-white/5 border border-white/10 text-white rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-2">
