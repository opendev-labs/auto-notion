    ...
    return (
        <div className="space-y-8">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <button className="flex items-center gap-3 px-8 py-4 bg-white text-black rounded-lg font-bold text-xs uppercase tracking-widest hover:bg-zinc-200 transition-all shadow-xl">
                    ...
                    <div className="glass-dark p-6 rounded-lg border border-white/10 space-y-6">
                        ...
                        <nav className="space-y-2">
                            {[
                                ...
                                <button key={i} className={`w-full flex items-center justify-between px-4 py-3 rounded-md transition-all ${item.active ? 'bg-white/10 text-white' : 'text-white/40 hover:bg-white/5'}`}>
                                    <span className="text-xs font-bold">{item.label}</span>
                                    <span className="text-[9px] font-black px-2 py-0.5 bg-white/5 rounded-md border border-white/5">{item.count}</span>
                                </button>
                            ))}
                        </nav>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                        <div className="relative flex-1 w-full max-w-sm">
                            ...
                            <input
                                ...
                                className="w-full bg-black/40 border border-white/10 rounded-lg pl-12 pr-4 py-3 text-xs text-white outline-none focus:border-white/40 transition-all font-medium"
                            />
                        </div>
                        <div className="flex gap-2">
                            <button className="p-3 bg-white/5 border border-white/10 rounded-lg text-white/40"><Grid size={16} /></button>
                            <button className="p-3 bg-white text-black rounded-lg"><List size={16} /></button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {materials.map((item) => (
                            <motion.div
                                key={item.id}
                                whileHover={{ y: -4 }}
                                className="glass-dark p-6 rounded-lg border border-white/10 group hover:border-white/20 transition-all"
                            >
                                <div className="flex items-start justify-between mb-6">
                                    <div className="p-3 bg-white/5 rounded-md text-white/60 group-hover:bg-white group-hover:text-black transition-all border border-white/5">
