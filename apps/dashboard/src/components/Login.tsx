...
import { googleProvider, facebookProvider, githubProvider, signInWithProvider, auth } from '../services/firebase';
import { Github, Facebook, Quote, CheckCircle2, Shield, Lock, ArrowRight, Loader2, User } from 'lucide-react';
    ...
    const handleLogin = async (provider: any, providerName: string) => {
        ...
        const handleDemoLogin = async () => {
            ...
            try {
                ...
                await signInWithEmailAndPassword(auth, 'demo@auto-notion.web.app', 'demo1234');
                onLoginSuccess();
            } catch (err: any) {
                console.error("Demo login error:", err);
                // If user doesn't exist, we could try to create it, or just show error
                if (err.code === 'auth/user-not-found' || err.code === 'auth/invalid-credential') {
                    setError('Demo account not found. Please create user "demo@auto-notion.web.app" with password "demo1234" in Firebase Console.');
                } else {
                    ...
                    >
                        ...
                        <div className="pt-4">
                            <button
                                ...
                                className="group relative w-full h-12 flex items-center justify-center px-6 bg-white/5 border border-white/10 text-white/60 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-white/10 hover:text-white transition-all disabled:opacity-50"
                            >
                                <div className="flex items-center gap-3">
                                    {loadingProvider === 'Demo' ? <Loader2 className="animate-spin" size={16} /> : <User size={16} />}
